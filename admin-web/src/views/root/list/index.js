...
const RootListView = (props) => {
    ...
    const [initLoaded, setInitLoaded] = useState(0)
    ...
    const onChangeColumnState = useCallback(() => {
        if (initLoaded === 0) { return }
        if (state.viewMode === ViewMode.Master || state.viewMode === ViewMode.Tree) {
            console.log(`[${state.viewMode}]`, 'save column state ', columnApi.current.getColumnState())
            localStorage.setItem(`__column_state__${state.viewMode}__`, JSON.stringify(columnApi.current.getColumnState()))
        }

    }, [state.viewMode, initLoaded])
    ...
    useEffect(() => {
        if (initLoaded === 0 || !state.viewMode) { return }

        let tm = setTimeout(() => {
            let savedColumnState = JSON5.parse(localStorage.getItem(`__column_state__${state.viewMode}__`))
            if (savedColumnState) {
                console.log(`[${state.viewMode}]`, 'load column state ', savedColumnState)
                columnApi.current.applyColumnState({state: savedColumnState, applyOrder: true})
            }
        }, 200)
        return () => {
            clearTimeout(tm)
